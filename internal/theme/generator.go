...
func (m *Manager) GenerateTheme(scheme, name string, save bool) error {
	...
	if name == "" {
		...
		name = fmt.Sprintf("%s_generated_%s", scheme, timestamp)
	}
...
// All the color generation methods remain the same from before...
func (m *Manager) generateRandomColors() map[string]string {
	colors := make(map[string]string)
	baseHue := randomFloat()

	// Background and foreground with good contrast
	bgLightness := randomFloat() * 0.15
	fgLightness := 0.85 + randomFloat()*0.15

	colors["background"] = HSL{H: baseHue, S: 0.1, L: bgLightness}.ToRGB().ToHex()
	colors["foreground"] = HSL{H: baseHue, S: 0.1, L: fgLightness}.ToRGB().ToHex()
	colors["selection_background"] = HSL{H: baseHue, S: 0.3, L: 0.3}.ToRGB().ToHex()
	...
	for i, name := range colorNames {
		...
		if name == "black" {
			light = randomFloat() * 0.2
			sat = 0.1
			hue = baseHue
		} else if name == "white" {
			light = 0.8 + randomFloat()*0.2
			sat = 0.1
			hue = baseHue
		} else {
			hue = math.Mod(baseHue+colorHues[i]+randomFloat()*0.1-0.05, 1.0)
			sat = 0.6 + randomFloat()*0.4
...
func (m *Manager) generatePastelColors() map[string]string {
	...
	colors["background"] = "#faf4ed"
	colors["foreground"] = "#575279"
	colors["selection_background"] = "#e6d6db"
	...
	for i, name := range colorNames {
		...
		if name == "black" {
			colors[name] = "#f2e9e1"
			colors["bright_"+name] = "#9893a5"
		} else if name == "white" {
			colors[name] = "#575279"
			colors["bright_"+name] = "#26233a"
		} else {
			hue = math.Mod(baseHue+pastelHues[i], 1.0)
			sat = 0.3 + randomFloat()*0.2
			light = 0.65 + randomFloat()*0.25

			colors[name] = HSL{H: hue, S: sat, L: light}.ToRGB().ToHex()
			colors["bright_"+name] = HSL{H: hue, S: sat + 0.1, L: math.Max(0.4, light-0.15)}.ToRGB().ToHex()
		}
	}
